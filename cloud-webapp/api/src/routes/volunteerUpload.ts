 ...
 *
 * TODO(UPLOAD_RESUMABLE_NOTES): add reCAPTCHA Enterprise on /session (the
 * same gate findme uses) + a per-token rate limit before public launch, so a
 * leaked link can't be used to fill the staging bucket.
 */
 ...
 */
volunteerUploadRouter.post('/volunteer/upload/session', async (req, res, next) => {
  try {
    ...
    const { token, fileName, mimeType, size } = parsed.data;
    ...
    const session = await createResumableSession(link, batchId, fileName, effectiveMime);
...
volunteerUploadRouter.post('/volunteer/upload/complete', async (req, res, next) => {
  try {
    ...
    const link = await validateUploadLink(token);
    const accepted = await enqueueStagedBatch(
      link,
      batchId,
    ...
    const body: CompleteUploadResponse = {
      ...
      batchId,
      accepted,
      message: `Received ${accepted} file${accepted === 1 ? '' : 's'} for "${link.eventName || link.eventId}".`,
    };
    res.json(body);
  } catch (err) {
    if (err instanceof UploadLinkError) {
