...
/**
 * Express middleware factory. Keys on the authenticated uid (falls back to ip).
 * Place AFTER requireAuth so `req.user` is populated.
 */
export function rateLimit(opts: { bucket: string; limit: number; windowSec: number }): RequestHandler {
