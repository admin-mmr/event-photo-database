 ...
 * `multiple_faces` — the census carries the face count itself, so the code
 * would be redundant there. Sharing the codes is deliberate: they come from one
 * `face_advisories()` in matcher/quality.py, so a searcher cannot be warned
 * "turned away" before searching and told nothing after.
 */
