...
- **`findme-folder-rebuild`** (`/api/admin/folders/rebuild-drain`, ~every 2 min —
  ...
  - **The api request timeout is 300s, NOT 60s** (`deploy-api.sh --timeout=300`).
    ...
    and skipped, trashed shortcuts stay trashed), so a killed run resumes; 300s
    lets a big event finish in one claim. Hosting-routed user paths still cap at
    60s (Firebase Hosting max), so only the direct run.app machine calls get the
    longer window; scale-to-zero / idle cost is unaffected.
