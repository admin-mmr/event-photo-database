 ...
 * unhandled promise rejection, or a download/save that fails for every photo)
 * never reach the server, so a log-based alert never fires. This helper POSTs
 * them to /api/client-errors, which logs them at ERROR severity. Mirrors
 * cloud-webapp/web/src/lib/reportError.ts. See api/src/routes/telemetry.ts.
 *
