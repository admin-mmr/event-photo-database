...
# Azure counterpart of the Cloud Scheduler backstop. A Container Apps JOB with a
# Schedule trigger POSTs /api/admin/index-scan on the api on a short interval;
# the endpoint triggers the photo-indexer job for every active event with new
# Drive files (idempotent — events with nothing new are a cheap no-op).
#
# Usage:
#   ./infra/scripts/provision-index-scan-scheduler.sh [resource-group] [location]
#
# Tunable: INDEX_SCAN_CRON (default every 10 min). Token read from Key Vault.
# Schedule jobs scale to zero between fires (no idle cost).
